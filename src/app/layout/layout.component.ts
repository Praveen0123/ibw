...
export class LayoutComponent implements OnInit { 
  ...
  userType: string = "";
  hasAlertsPermission: boolean=false;
  hasReviewsPermission: boolean=false;
  hasDashboardPermission: boolean=false;
  isSuperAdmin: boolean=false;

  constructor(public appSettings:AppSettings, public router:Router, private menuService: MenuService){        
    this.settings = this.appSettings.settings;
    var userSession = JSON.parse(localStorage.getItem('userloginsession'));
    var userRights = userSession.UserRights;
    if(userRights.length > 0){
      if(userRights.filter(x=>x.vc_module == 'Dashboard').length>0){
        this.hasDashboardPermission=true;
      }
      if(userRights.filter(x=>x.vc_module == 'Layout' && x.vc_screen_name == 'View alerts').length>0){
        this.hasAlertsPermission=true;
      }
      if(userRights.filter(x=>x.vc_module == 'Layout' && x.vc_screen_name == 'View reviews').length>0){
        this.hasReviewsPermission=true;
      }
    
    
    }
    if(userSession.Roles.length > 0){
    if(userSession.Roles.filter(x=> x == 'Super Admin').length > 0){
