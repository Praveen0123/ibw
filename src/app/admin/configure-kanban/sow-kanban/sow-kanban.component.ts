...
@Component({
  selector: 'app-sow-kanban',
  templateUrl: './sow-kanban.component.html',
  styleUrls: ['./sow-kanban.component.scss']
})
export class SowKanbanComponent implements OnInit {
  public steps : any;
  ...
  public getKanbanStages() {
    this._configureKanabnSerive.getKanbanStages().subscribe(data => {
      if(data['Data'].length > 0){
        this.kanbanStepsData = data['Data'].filter(x => x.KanbanTypeName == "sow");
      }
